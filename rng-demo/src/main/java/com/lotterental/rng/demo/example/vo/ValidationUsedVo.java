...
public class ValidationUsedVo extends BaseMetaVo {
	...
	@RngNotEmpty(messageCode = "CODE0002", messageParams = {"이름", "필수"}, order = 2)	// name != null && name.length() > 0
	private String name;
	
	@RngNotBlank(messageCode = "CODE0003", messageParams = {"전공", "필수"})				// major != null && major.length().trim() > 0
	...
	@RngSize(min = 40, max = 150, messageCode = "CODE0006", messageParams = "시력")		// 40 <= weight <= 150
	private int weight;
}
